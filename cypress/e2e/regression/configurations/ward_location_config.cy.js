...
import { wardConfigPage } from '../../../support/pageObjects/AdminPortalPageObjects/WardConfigPage'
...
describe('LocationConfig', () => {
	...
	const ward = locationConfigs.automationWard

	before('Activate country, state , city and LGA via API', () => {
		cy.updateCountryStatus(country)
		cy.updateStateStatus(state)
		cy.updateLgaStatus(lga)
	})
	beforeEach('login', () => {
		cy.login()
	})
	...
	it('Create Ward', () => {
		...
		wardConfigPage.clickViewWard()
		adminCustomers.searchListingTable(ward.name)
		cy.wait(2000)
		adminCustomers.checkBusinessExists(ward.name).then((exists) => {
			if (exists) {
				cy.log(`${ward.name} already exists. Skip creation`)
				return
			} else {
				//lgaConfigPage.clickCreateLga()
				wardConfigPage.clickCreateWard()
				...
				wardConfigPage.clickLgaDropdown()
				wardConfigPage.selectLGA(lga.name, city)
				wardConfigPage.enterWardName(ward.name)
				wardConfigPage.enterWardCode(ward.code)
				locationConfigPage.clickSubmitCountryBtn()
				adminCustomers.verifySuccessModal('You have successfully created a new Ward')
				locationConfigPage.verifyCountryExists(ward.name)
			}
		})
	})
